...
@contextmanager
def track_block(name, device=None):
    ...
    if device.type == 'cuda':
        ...
        peak = peak_mem / 1024**2
        print(f"[{name}] elapsed {elapsed:.3f}s | Δalloc {delta:.1f} MB | peak {peak:.1f} MB")
    else:
        print(f"[{name}] elapsed {elapsed:.3f}s (CPU only)")
...
class LFAN(nn.Module):
    ...
    def load_visual_backbone(self, backbone_settings):
        if self.visual_backbone_type == "resnet50":
            ...
            # with track_block("Load ResNet50 Backbone", self.device):
            #     backbone = my_res50(mode='ir', use_pretrained=False)
            ...
            with track_block("Load Visual Backbone Small", self.device):
